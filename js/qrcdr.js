  ...
  $.fn.extend({
    ...
    qrcdr: function (options) {
      ...
      var $myForm,
        ...
        generate_qrcode_btn,
        holdresult
      var collapse_control,
        collapse_control_reverse,
        upmarker,
        isSvg,
        event,
        map,
      ...
      return this.each(function () {
        ...
        formOnInput = obj.find(
          '.qrcdr-form :input[type="text"], .qrcdr-form textarea',
        )
        formOnChange = obj.find(
          '.qrcdr-form :input:not([type="text"]):not(textarea), .qrcolorpicker :input',
        )
        ...
        var validation = Array.prototype.filter.call(validateforms, function (
          form,
        ) {
        ...
        function printIt(printThis) {
          ...
          win.document.open()
          win.document.write(
            '<html><head></head><body onload="window.print()" onafterprint="window.close()"><img src="' +
              src +
              '"/>' +
              infopanel +
              '</body></html>',
            ...
            .done(function (msg) {
              if (msg.length) {
                ...
                if (getdata.error) {
                  ...
                  var downloadlinks =
                    '<button class="btn btn-default svgtopng" data-path="' +
                    filepath +
                    '">' +
                    pngIcon +
                    '</button><a href="#" class="btn btn-default d-none preload-png">' +
                    loadIcon +
                    '</a><a class="serve-png d-none" href="' +
                    filepath +
                    '.png" download="' +
                    getdata.basename +
                    '.png" data-path="' +
                  ...
                  downloadlinks =
                    downloadlinks +
                    '<a class="btn btn-default serve-svg" href="' +
                    filepath +
                    '.svg" download="' +
                    getdata.basename +
                    '.svg">' +
          ...
          if (this.files[0].type.match('image.*')) {
            ...
            reader.onload = function (e) {
              $('.logoselecta label')
                ...
                .removeAttr('checked')
              var out =
                '<img src="' + e.target.result + '" class="user_watermark">'
              // Update custom watermark option
              $('.custom-watermark .hold-custom-watermark').html(out)
              newimg.src = $('.logoselecta .btn-group-toggle label')
                .last()
                .find('img')
                .attr('src')
            ...
            newimg.onload = function () {
              ...
              if (isSvg !== 'svg') {
                ...
                $('.logoselecta .btn-group-toggle label')
                  .last()
                  .find('img')
                  .attr('src', dataurl)
                $('.logoselecta .btn-group-toggle label')
                  .last()
                  .find("input[name='optionlogo']")
                  .val(dataurl)
              }
            } // img.onload
          } else {
